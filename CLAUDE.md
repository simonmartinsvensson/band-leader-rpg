...
(`npm run gen:map`) — `town`, `street`, `studio`, `jazz_club`, `vip_lounge`, `park`,
`warehouse`, and `backstage`.
...
`warehouse`/`backstage` — are covered under Venues, trainers & residencies.)
...
- **Real encounters** — stepping in an encounter zone rolls `rollEncounter(zone)`; on a hit the
  ...
  step-up zone east of town, home to the warehouse counters.)
...
- `npm run gen:map` — regenerate the sample Tiled maps (`town`, `street`, `studio`, `jazz_club`,
  `vip_lounge`, `park`, `warehouse`, `backstage`) in `src/data/maps`.
