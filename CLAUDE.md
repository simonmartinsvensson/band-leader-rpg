...
`char_*.png`); tiles, the battle musician placeholders (`player`/`npc`), and the bitmap font are
still generated placeholders. Loader keys and frame indices are defined once in
`src/data/assets.ts` — always reference assets by these keys, never by raw path.
...
| `player`     | `assets/player.png`    | spritesheet | **Battle musician placeholder only** — `0` down…`3` right (`PlayerFrame`), 16×16 |
| `npc`        | `assets/npc.png`       | image       | **Battle musician placeholder only** — single 16×16 frame |
...
`scripts/repack-characters.py` (`npm run gen:characters`). The `player`/`npc` placeholder PNGs
(`gen:assets`) are now used **only** by the battle scene for musician sprites (still placeholders —
next job). The tileset (`gen:assets`) and font (`gen:font`) are still generated placeholders.
...
| `player`   | `assets/player.png`   | spritesheet | **64×16**  | PNG RGBA        | **Battle musician placeholder only.** 4 frames of 16×16: `0` down, `1` up, `2` left, `3` right (`PlayerFrame`). |
| `npc`      | `assets/npc.png`      | image       | **16×16**  | PNG RGBA        | **Battle musician placeholder only.** Single generic frame, tinted by genre color at runtime. |
...
- **Musicians (battle) have no sprites of their own.** `BattleScene` renders the opponent with the
  `npc` placeholder and the player's active musician with `player` frame 0, each **tinted by genre
  color** (`genreColor`). Real musician art is *not* a drop-in — it needs per-species sprites (~50),
  a loader entry per sprite (or a packed sheet + a `spriteFrame` field on `MusicianSpecies`), and a
  `BattleScene` change. **That's the next job.** Until then, `player.png`/`npc.png` stay placeholders.
